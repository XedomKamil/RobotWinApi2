...
namespace WindowsFormsApp
{
    ...
    public partial class WinAppForm : System.Windows.Forms.Form
    {
        ...
        // Wczytanie ikonek
        Image imageFolder = WindowsFormsApp2.Properties.Resources.folder.GetThumbnailImage(120, 90, null, IntPtr.Zero);
        Image imageLoading = WindowsFormsApp2.Properties.Resources.Loading.GetThumbnailImage(120, 90, null, IntPtr.Zero);
        ...
        List<List<double>> charTemps = new List<List<double>>();
        WindowsFormsApp2.FormChart secondForm = new WindowsFormsApp2.FormChart();
            ...
            if (trackbarLastVal != trackbarWithLabel1.Trackbar.Value)
            {
                ...
                if (trackbarWithLabel1.Trackbar.Value <= imagesList2.Count - 1)
                    if (trackbarWithLabel1.Trackbar.Value >= 0)
                    {
                        try
                        {
                            label2.Text = Convert.ToString(gUIElements.times[trackbarWithLabel1.Trackbar.Value]);
                        }
                        catch { }
        ...
        private void pictureBoxIpl1_DoubleClick(object sender, EventArgs e)
        {
            ...
            if (tempArrayD.Count > 0)
            {
                ...
                // WindowsFormsApp2.FormChart secondForm = new WindowsFormsApp2.FormChart(charTemps);
