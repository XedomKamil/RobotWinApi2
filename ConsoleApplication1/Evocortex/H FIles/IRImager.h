...
public:
   ...
   * @return missing files as comma separated list, free returned pointer with delete[].
   */
   ...
   */
  bool getTemperatureTableParameters(double* t1, double* t3, double* tJunct, short* e1, short* e3, short* eJunct, double* lambda, double* lambda2);
   ...
   * Internal method not to be used by any application!
   */
  void onSnapshotEvent(unsigned short* thermal, unsigned short* energy, unsigned char* rgb, IRFrameMetadata meta, std::vector<IREventData> events);
   ...
   */
  Point getClippedFormatPosition();
   ...
   */
  unsigned short getPifAOCount();

  /**
   * @brief Get the Pif Device Count
   * @return unsigned char device count
   */
  unsigned char getPifDeviceCount(bool actual);
...
private:
  ...
  void checkResult(long result, const char* functionName);

  void onAutoSkimOperation(bool ready);
